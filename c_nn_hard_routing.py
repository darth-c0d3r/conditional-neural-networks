...
class c_nn(nn.Module):
	...
	def __init__(self, size, conv, tree, f_c, n_classes, n_children, n_pass, feat_dropout):
	...
	def forward(self, x):
		...
		for i in range(self.tree_depth):
			for j in range(len(x)):
				...
				r = F.softmax(self.routers[2**i + j - 1](x[j]))
				for k in range(r.size()[0]):
					maxies = torch.topk(r[k],self.n_pass)[1]
					for index in range(len(x_stack)):
						if index in maxies:
							x_stack[index].append(x[j][k])
						else:
							x_stack[index].append(torch.zeros(x[j][k].size()))
				for k in range(len(x_stack)):
					x_stack[k] = torch.stack(x_stack[k])

				x_cummulative = []
				for k in range(self.n_children):
					x_temp = F.relu(self.tree_layer[current_index](x_stack[k]))
